...
export default function ReportsPage() {
  ...
  const areasQuery = useQuery({
    queryKey: ['governance-areas-stats', Number(year)],
  ...
  // ── Derived: office leaderboard ───────────────────────────────────────────────
  const activeAreas = useMemo(
    () => (areasQuery.data?.governanceAreas || []).filter((a) => a.is_active),
    [areasQuery.data]
  );
  ...
  const officeLeaderboard = useMemo(() => {
    if (!activeAreas.length || !offices.length) return [];
    const areasToCheck = activeAreas;
    return offices
      .map((o) => {
        const t = areasToCheck.length;
        const approved = areasToCheck.filter(
          (a) => matrix[a.id]?.[o.id] === 'APPROVED'
        ).length;
        const pct = t > 0 ? Math.round((approved / t) * 100) : 0;
        return { id: o.id, name: o.name, approved, total: t, pct };
      })
      .sort((a, b) => b.pct - a.pct);
  }, [activeAreas, offices, matrix]);
  ...
  const generatedAt = useMemo(() => {
    ...
    return ts
      ...
      : null;
  }, [activeTab, overviewQuery.dataUpdatedAt, summaryQuery.dataUpdatedAt, missingQuery.dataUpdatedAt, matrixQuery.dataUpdatedAt]);

  // ── Refresh ───────────────────────────────────────────────────────────────────
  const isRefreshing =
    overviewQuery.isFetching || summaryQuery.isFetching ||
    missingQuery.isFetching  || areasQuery.isFetching   || matrixQuery.isFetching;
  ...
  const handleRefresh = () => {
    ...
    else if (activeTab === 'rankings') { areasQuery.refetch(); matrixQuery.refetch(); }
  };
  ...
  const exportLeaderboardCSV = () => {
    if (!officeLeaderboard.length) return;
    const ctx = buildContextHeader('Office Compliance Rankings', year, 'All Offices');
    const header = 'Rank,Office,Approved Areas,Total Areas,Compliance %\n';
    const body = officeLeaderboard
      .map((o, i) => `${i + 1},"${o.name}",${o.approved},${o.total},${o.pct}%`)
      .join('\n');
    downloadCSV(`office-rankings-${year}.csv`, ctx + header + body);
  };
        ...
        {isAdminOrStaff && (
          <TabsContent value="rankings" className="space-y-6 mt-6">
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-3">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Office Compliance Rankings
                  </CardTitle>
                  <CardDescription>
                    Governance area approval rate per office for {year}
                  </CardDescription>
                </div>
                {officeLeaderboard.length > 0 && (
                  <Button variant="outline" size="sm" onClick={exportLeaderboardCSV}>
                    <Download className="h-4 w-4 mr-2" />
                    Export CSV
                  </Button>
                )}
              ...
              <CardContent>
                {(areasQuery.isFetching || matrixQuery.isFetching) && !officeLeaderboard.length ? (
                  <div className="flex items-center justify-center py-14 text-muted-foreground">
                    <Loader2 className="h-6 w-6 animate-spin mr-3" /> Loading rankings…
                  </div>
                ) : officeLeaderboard.length === 0 ? (
                  <EmptyState title="No data available" sub="No offices or governance areas found for this year." />
                ) : (
                  <div className="space-y-6">
                    {/* Top 3 podium highlight */}
                    {officeLeaderboard.length >= 3 && (
                      <div className="grid grid-cols-3 gap-3">
                        {[officeLeaderboard[1], officeLeaderboard[0], officeLeaderboard[2]].map((o, podiumPos) => {
                          ...
                          return (
                            <div
                              key={o.id}
                              className={cn(
                                'flex flex-col items-center justify-end rounded-lg border-2 px-3 pb-3 pt-2 gap-1 transition-all',
                                heights[podiumPos],
                                colors[podiumPos]
                              )}
                            >
                              ...
                              <span className="text-lg font-extrabold tabular-nums">{o.pct}%</span>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="overflow-x-auto -mx-6">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b bg-muted/40">
                            <th className="px-6 py-2.5 text-left font-medium text-xs text-muted-foreground w-12">Rank</th>
                            <th className="px-3 py-2.5 text-left font-medium text-xs text-muted-foreground">Office</th>
                            <th className="px-3 py-2.5 text-center font-medium text-xs text-muted-foreground w-28">Approved / Total</th>
                            <th className="px-6 py-2.5 text-right font-medium text-xs text-muted-foreground w-24">Compliance</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y">
                          {officeLeaderboard.map((o, i) => {
                            const rank       = i + 1;
                            const isTop3     = rank <= 3;
                            const rankColors = ['text-yellow-600', 'text-slate-500', 'text-orange-600'];
                            const barColor   =
                              o.pct >= 80 ? 'bg-green-500' :
                              o.pct >= 50 ? 'bg-amber-400' : 'bg-red-400';
                            return (
                              <tr
                                key={o.id}
                                className={cn(
                              ...
                              >
                                <td className={cn('px-6 py-3 font-bold text-center', isTop3 ? rankColors[i] : 'text-muted-foreground')}>
                                  {rank}
                                </td>
                                ...
                                <td className="px-6 py-3">
                                  <div className="flex items-center gap-2 justify-end">
                                    ...
                                    <span className={cn(
                                      ...
                                      o.pct >= 80 ? 'text-green-700' : o.pct >= 50 ? 'text-amber-700' : 'text-red-600'
                                    )}>
                                      {o.pct}%
                                    </span>
                                  </div>
